...
void FileTools::Obj::ParseObjFile(
	...
	std::vector<MeshInfo> *pMeshOffsetData){
	...
	size_t vertPosBufferOffset = 0;
	size_t vertNormalBufferOffset = 0;
		...
		for(auto lineSv:lines){
			...
			case HeaderCode::face:
			{
				...
				Obj::LoadVertexData(vertTokenList, vertPosBuffer,
					texCoordBuffer, vertNormalBuffer, vertPosBufferOffset,
					texCoordBufferOffset, vertNormalBufferOffset, &vert0);
				...
				for(size_t i = 0; i<numTriangles; ++i){
					...
					for(size_t j = 0; j<2; ++j){
						...
						Obj::LoadVertexData(vertTokenList, vertPosBuffer,
							texCoordBuffer, vertNormalBuffer, vertPosBufferOffset,
							texCoordBufferOffset, vertNormalBufferOffset, &vertData);
			...
			case HeaderCode::group:
			{
				if(!interleavedBuffer.empty()){
					pMeshOffsetData->push_back({groupName, pIndexBuffer->size(), interleavedBuffer.size(), pIndexedVertexBuffer->size(), 0, 0.0f, AABB()});
...
void FileTools::Obj::LoadVertexData(
	...
	pVertData->vert = vertPosBuffer.at(vertIdx-vertPosBufferOffset);
	...
	if(texCoordBuffer.size()>0){
		...
		pVertData->texCoord = texCoordBuffer.at(texCoordIdx-texCoordBufferOffset);
	}
	...
	if(vertNormalBuffer.size()>0){
		...
		pVertData->normal = vertNormalBuffer.at(normIdx- vertNormalBufferOffset);
	}
}
